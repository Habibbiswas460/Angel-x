...
class AngelXIntegration:
    ...
    def get_performance_metrics(self) -> dict:
        ...
        try:
            ...
            return {
                "summary": summary,
                "best_entries": sorted(entry_analysis.items(), key=lambda x: x[1]["win_rate"], reverse=True)[:5],
                "best_exits": sorted(exit_analysis.items(), key=lambda x: x[1]["total_pnl"], reverse=True)[:5],
            }
        except Exception as e:
