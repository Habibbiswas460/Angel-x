...
class PaperTradingEngine:
    ...
    def place_order(
        ...
        if self.available_margin < margin_required:
            order.status = OrderStatus.REJECTED
            order.rejected_reason = f"Insufficient margin. Required: ₹{margin_required:,.2f}, Available: ₹{self.available_margin:,.2f}"
            self.orders[order_id] = order
    ...
    def _close_position(self, sell_order: PaperOrder) -> bool:
        ...
        logger.info(
            f"✓ Position Closed | {symbol} | P&L: ₹{pnl_rupees:+.2f} ({pnl_percent:+.2f}%)"
        )
