...
class PaperTradingJournal:
    ...
    def export_csv(self, filename: Optional[str] = None) -> str:
        ...
        # CSV headers
        headers = [
            'trade_id', 'timestamp', 'symbol', 'option_type', 'strike', 'quantity',
            'entry_time', 'entry_price', 'entry_delta', 'entry_gamma', 'entry_theta', 'entry_vega', 'entry_iv',
            'exit_time', 'exit_price', 'exit_delta', 'exit_gamma', 'exit_reason',
            'pnl_amount', 'pnl_percent', 'time_in_trade_seconds',
            'entry_spread', 'exit_spread', 'entry_reason_tags', 'exit_reason_tags', 'slippage'
        ]
...
"""
        ...
        if sorted_trades:
            ...
            for i, trade in enumerate(sorted_trades[:5], 1):
                report += f"  {i}. {trade['symbol']} | P&L: ₹{trade['pnl_amount']:+,.2f} ({trade['pnl_percent']:+.2f}%)\n"
