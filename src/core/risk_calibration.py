...
class RiskCalibrationEngine:
    ...
    def get_calibrated_risk_profile(self) -> RiskProfile:
        ...
        profile.max_trades_per_day = int(profile.max_trades_per_day * 
                                        self.time_manager.get_trade_limit_multiplier())
    ...
    def can_take_trade(self) -> Dict:
        ...
        if self.drawdown_protection.should_stop_trading():
            return {"allowed": False, "reason": f"Max drawdown exceeded: {self.drawdown_protection.get_drawdown_pct():.1f}%"}
