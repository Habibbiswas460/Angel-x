...
class PositionSizing:
    ...
    def estimate_win_probability(
        self, delta: float, gamma: float, iv: float, bias_confidence: float, oi_change: int = 0
    ) -> float:
        """
        ...
        logger.debug(
            f"Win probability: {prob:.2%} (Δ={delta:.3f}, Γ={gamma:.4f}, " f"IV={iv:.1f}%, conf={bias_confidence:.0f})"
        )
    ...
    def calculate_position_size(
        ...
        if hard_sl_price is None:
            return PositionSize(
                ...
                risk_reward_ratio=0,
                sizing_valid=False,
                rejection_reason="No SL provided",
            )
        """
        Calculate optimal position size based on risk parameters + dynamic factors
        ...
        if self.use_probability_weighting and all(x is not None for x in [delta, gamma, iv, bias_confidence]):
            win_prob = self.estimate_win_probability(
                delta=delta, gamma=gamma, iv=iv, bias_confidence=bias_confidence, oi_change=oi_change or 0
            )
    ...
    def get_recommendation(
        ...
        sizing = self.calculate_position_size(
            entry_price, sl_price, target_price, risk_percent, expiry_rules=expiry_rules
        )
