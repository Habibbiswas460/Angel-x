...
class ExpiryManager:
    ...
    def __init__(self):
        ...
        if api is None:
            ...
            try:
                self.client = api(
                    api_key=config.OPENALGO_API_KEY, host=config.OPENALGO_HOST, ws_url=config.OPENALGO_WS_URL
                )
    ...
    def fetch_available_expiries(self, underlying: str) -> List[ExpiryInfo]:
        ...
        try:
            ...
            if hasattr(self.client, "getoptionchain"):
                response = self.client.getoptionchain(exchange="NFO", symbol=underlying)
            elif hasattr(self.client, "get_option_chain"):
                response = self.client.get_option_chain(exchange="NFO", symbol=underlying)
            else:
            ...
            for exp_date_str in sorted(expiry_dates):
                try:
                    ...
                    expiry_info = ExpiryInfo(expiry_date=exp_date_str, expiry_type=exp_type, days_to_expiry=days_to_exp)
    ...
    def _get_default_expiries(self) -> List[ExpiryInfo]:
        ...
        for i, exp in enumerate(expiry_list):
            ...
            logger.info(
                f"  Week {i+1}: {exp.expiry_date} - {exp_date.strftime('%A, %d %B %Y')} ({exp.days_to_expiry} days)"
            )
    ...
    def get_expiry_statistics(self) -> Dict:
        ...
        return {
            ...
            "trading_rules_applied": (
                "expiry_day" if exp.is_last_day else "expiry_week" if exp.is_expiry_week else "normal"
            ),
        }
    ...
    def get_option_symbol(self, strike: int, option_type: str, underlying: str = None) -> str:
        """
    ...
    def get_option_symbol_by_offset(
        self, underlying: str, expiry_date: str, offset: str, option_type: str
    ) -> Optional[str]:
        """Resolve option symbol using OpenAlgo `optionsymbol` via OrderManager."""
        try:
            resp = self._order_manager.resolve_option_symbol(
                underlying=underlying, expiry_date=expiry_date, offset=offset, option_type=option_type
            )
            if resp and resp.get("status") == "success":
                symbol = resp.get("symbol")
                logger.log_order(
                    {"type": "SYMBOL_RESOLVED", "symbol": symbol, "offset": offset, "expiry_date": expiry_date}
                )
                return symbol
