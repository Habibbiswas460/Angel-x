...
class DataFreezeDetector:
    ...
    def can_trade(self) -> Dict:
        ...
        if staleness > self.max_staleness_seconds:
            return {
                'allowed': False,
                'reason': f'Data stale ({staleness:.1f}s old, max {self.max_staleness_seconds}s)'
            }
        
        if self.freeze_detected:
            return {
                'allowed': False,
                'reason': 'Data freeze detected'
            }
        
        if self.trading_blocked:
            return {
                'allowed': False,
                'reason': self.block_reason or 'Trading blocked'
            }
        
        # All checks passed
        return {
            'allowed': True,
            'reason': f'Data fresh ({staleness:.1f}s old)'
        }
...
class PartialOrderReconciliation:
    ...
    def update_fill(self, order_id: str, filled_qty: int):
        ...
        if order_id in self.pending_orders:
            ...
            self.partial_fills[order_id].append({
                "qty": filled_qty,
                "timestamp": datetime.now()
            })
...
class SafeExitManager:
    ...
    def mark_position_exited(self, position_id: str):
        ...
        self.positions_to_exit = [
            p for p in self.positions_to_exit 
...
class FailoverRecoverySystem:
    ...
    def should_allow_trading(self) -> Dict:
        ...
        if not self.broker_monitor.can_trade():
            return {
                "allowed": False,
                "reason": "Broker connection issues - system paused"
            }
        ...
        if self.freeze_detector.is_data_frozen():
            return {
                "allowed": False,
                "reason": f"Data frozen for {self.freeze_detector.get_staleness_seconds()}s"
            }
        ...
        if self.safe_exit.exit_mode:
            return {
                "allowed": False,
                "reason": f"Safe exit mode: {self.safe_exit.exit_reason}"
            }
        ...
        if self.state in [SystemState.CRITICAL, SystemState.PAUSED]:
            return {
                "allowed": False,
                "reason": f"System state: {self.state.value}"
            }
