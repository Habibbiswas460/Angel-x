...
class TradeManager:
    ...
    def enter_multi_leg_order(
        self, underlying: str, legs: List[dict], expiry_date: Optional[str] = None
    ) -> Optional[dict]:
        ...
        """
        try:
            intent = {"type": "MULTI_LEG_INTENT", "underlying": underlying, "expiry_date": expiry_date, "legs": legs}
            logger.log_order(intent)
            resp = self._order_manager.place_options_multi_order(
                strategy=config.STRATEGY_NAME, underlying=underlying, legs=legs, expiry_date=expiry_date
            )
            if resp and resp.get("status") == "success":
                ...
                if not results or len(results) == 0:
                    if config.PAPER_TRADING:
                        ...
                        resp["results"] = [
                            {"status": "success", "orderid": resp.get("orderid"), "leg": leg} for leg in legs
                        ]
    ...
    def update_trade(
        ...
        trade.pnl_percent = (
            ((current_price - trade.entry_price) / trade.entry_price * 100) if trade.entry_price > 0 else 0
        )
