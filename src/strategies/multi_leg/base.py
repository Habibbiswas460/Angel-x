...
class Leg:
    """
    Simplified leg model for tests and high-level strategy composition.

    Matches the expectations in tests (instrument, strike, kind, side, etc.).
    """
    ...
    def __str__(self) -> str:
        return f"{self.side} {self.qty}x {self.instrument} " f"({self.kind} @ {self.entry_price:.2f})"
...
class OptionLeg:
    ...
    @property
    def kind(self) -> str:
        """Alias for option_type for test compatibility. Returns enum name (CALL/PUT)."""
        return self.option_type.name

    @property
    def side(self) -> str:
        """Alias for direction for test compatibility. Returns BUY or SELL."""
        return self.direction.value

    def get_pnl(self) -> float:
        """Calculate profit and loss for this option leg.
        
        For BUY legs: (current_price - entry_price) * quantity
        For SELL legs: (entry_price - current_price) * quantity
        
        Returns:
            P&L value in rupees (positive = profit, negative = loss)
        """
...
class MultiLegStrategy(ABC):
    ...
    def __init__(
        ...
        """
        # Respect class attribute if set; otherwise use the passed name
        if not hasattr(self.__class__, "name") or self.__class__.name == "MultiLegStrategy":
            self.name = name
        else:
    ...
    def build_legs(self, spot: float, **kwargs) -> List[OptionLeg]:
        """Compatibility wrapper expected by tests; delegates to setup()."""
        return self.setup(spot_price=spot, **kwargs)
    ...
    def __str__(self) -> str:
        ...
        return (
            f"{self.name} [{status}] | " f"Legs: {len(self.legs)} | " f"P&L: ₹{pnl:.2f} | " f"Δ: {self.net_delta:.2f}"
        )
