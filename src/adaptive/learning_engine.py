...
class LearningEngine:
    ...
    def ingest_trade(self, trade_features: TradeFeatures):
        ...
        for bucket in buckets:
            ...
            recent_trades = [t for t in self.trade_history[-10:] if bucket in self._get_trade_buckets(t)]
    ...
    def _get_trade_buckets(self, trade: TradeFeatures) -> List[FeatureBucket]:
        ...
        return [trade.time_bucket, trade.bias_bucket, trade.greeks_bucket, trade.oi_bucket, trade.vol_bucket]
    ...
    def _analyze_time_buckets(self) -> List[LearningInsight]:
        ...
        # Find best and worst
        valid_buckets = [
            (b, self.bucket_performance[b]) for b in time_buckets if self.bucket_performance[b].sample_size_adequate
        ]
    ...
    def _analyze_oi_conviction(self) -> List[LearningInsight]:
        ...
        oi_buckets = [FeatureBucket.OI_STRONG, FeatureBucket.OI_MEDIUM, FeatureBucket.OI_WEAK]
            ...
            if high_gamma.win_rate >= 0.65:
                insights.append(
                    LearningInsight(
                        ...
                        supporting_data={"trades": high_gamma.total_trades, "win_rate": high_gamma.win_rate},
                        timestamp=datetime.now(),
                    )
                )
        ...
        if high_theta.sample_size_adequate and high_theta.win_rate >= 0.65:
            insights.append(
                LearningInsight(
                    ...
                    supporting_data={"trades": high_theta.total_trades, "win_rate": high_theta.win_rate},
                    timestamp=datetime.now(),
                )
            )
        ...
        if high_vol.sample_size_adequate and high_vol.win_rate <= 0.40:
            insights.append(
                LearningInsight(
                    ...
                    supporting_data={"trades": high_vol.total_trades, "win_rate": high_vol.win_rate},
                    timestamp=datetime.now(),
                )
            )
    ...
    def _analyze_combinations(self) -> List[LearningInsight]:
        ...
        strong_combos = [
            (combo, perf)
            for combo, perf in self.combo_performance.items()
        ...
        for combo, perf in strong_combos[:3]:  # Top 3
            ...
            insights.append(
                LearningInsight(
                    ...
                    supporting_data={"combo": bucket_names, "trades": perf.total_trades, "win_rate": perf.win_rate},
                    timestamp=datetime.now(),
                )
            )
