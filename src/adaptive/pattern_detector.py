...
class LossPatternDetector:
    ...
    def _update_blocks(self, patterns: List[LossPattern]):
        ...
        for pattern in patterns:
            if pattern.should_block():
                ...
                already_blocked = any(b.pattern.characteristic == pattern.characteristic for b in self.active_blocks)
    ...
    def get_worst_patterns(self, top_n: int = 3) -> List[LossPattern]:
        ...
        return sorted(self.detected_patterns, key=lambda p: p.total_loss, reverse=True)[:top_n]
