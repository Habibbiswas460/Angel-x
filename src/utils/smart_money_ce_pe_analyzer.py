...
class CePeBattlefieldAnalyzer:
    ...
    def analyze_battlefield(
        ...
        logger.debug(
            f"Battlefield: {control.value} | "
            f"CE_OI: {ce_oi_dominance:.2f} | "
            f"War_Intensity: {war_intensity:.2f}"
        )
    ...
    def _filter_atm_zone(
        ...
        filtered = [
            strike for strike in strikes_data.keys()
            if abs(strike - atm_ref) <= zone_range
        ]
    ...
    def get_metrics(self) -> Dict:
        ...
        return {
            ...
            "bullish_percentage": (
                self.bullish_dominance_count / max(total, 1) * 100
            ),
            "bearish_percentage": (
                self.bearish_dominance_count / max(total, 1) * 100
            ),
        }
