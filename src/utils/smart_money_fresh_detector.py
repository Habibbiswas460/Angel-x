...
class FreshPositionDetector:
    ...
    def detect_fresh_position(
        ...
        if oi_jump >= self.config.fresh_position_oi_jump and volume_surge >= self.config.fresh_position_volume_surge:
        ...
        if (
            previous_oi is None or previous_oi < self.config.first_time_oi_threshold
        ) and current_oi >= self.config.first_time_oi_threshold:
        ...
        logger.debug(
            f"Fresh position detected at {strike} {option_type}: " f"{entry_type} (confidence={confidence:.2f})"
        )
    ...
    def get_metrics(self) -> Dict:
        ...
        return {
            ...
            "detection_rate": (self.fresh_positions_detected / max(self.snapshots_analyzed, 1) * 100),
        }
