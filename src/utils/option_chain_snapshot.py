...
from src.utils.option_chain_data_models import (
    OptionChainSnapshot, OptionChainDelta, StrikeData, OptionType
)
...
class SnapshotEngine:
    ...
    def update_snapshot(self, new_snapshot: OptionChainSnapshot) -> OptionChainDelta:
        ...
        if delta.has_changes:
            self.logger.debug(f"Snapshot delta: {len(delta.oi_changes)} OI changes, "
                             f"{len(delta.volume_changes)} volume changes")
        
        return delta
    
    def _calculate_delta(self, current: OptionChainSnapshot, 
                        previous: Optional[OptionChainSnapshot]) -> OptionChainDelta:
        ...
        for strike in current_strikes & previous_strikes:
            ...
            if current_pair.ce and previous_pair.ce:
                self._check_strike_changes(
                    current_pair.ce, previous_pair.ce,
                    f"{strike}-CE", delta
                )
            ...
            if current_pair.pe and previous_pair.pe:
                self._check_strike_changes(
                    current_pair.pe, previous_pair.pe,
                    f"{strike}-PE", delta
                )
    ...
    def _check_strike_changes(self, current: StrikeData, previous: StrikeData,
                             strike_key: str, delta: OptionChainDelta):
    ...
    def _find_stale_strikes(self, current: OptionChainSnapshot,
                           previous: Optional[OptionChainSnapshot]) -> list:
        ...
        for strike in current.strikes.keys():
            ...
            ce_stale = (curr_pair.ce and prev_pair.ce and
                       curr_pair.ce.oi == prev_pair.ce.oi and
                       curr_pair.ce.volume == prev_pair.ce.volume and
                       curr_pair.ce.ltp == prev_pair.ce.ltp)
            
            pe_stale = (curr_pair.pe and prev_pair.pe and
                       curr_pair.pe.oi == prev_pair.pe.oi and
                       curr_pair.pe.volume == prev_pair.pe.volume and
                       curr_pair.pe.ltp == prev_pair.pe.ltp)
    ...
    def get_history_summary(self) -> Dict:
        ...
        return {
            ...
            'avg_fetch_latency_ms': sum(s.fetch_latency_ms for s in self.history) / len(self.history) if self.history else 0
        }
