...
from src.utils.option_chain_filters import NoiseFilter, DataValidator, StaleDataDetector, BrokerHiccupDetector
from src.utils.option_chain_snapshot import SnapshotEngine, SnapshotCache, SnapshotValidator
...
class OptionChainDataEngine:
    ...
    def __init__(self, adapter, config=None):
        ...
        # Health tracking
        self.health_report = DataHealthReport(status=DataHealthStatus.OFFLINE, timestamp=datetime.utcnow())
        self.fetch_count = 0
        self.error_count = 0
    ...
    def set_universe(self, underlying: str, expiry_date: datetime, atm_reference: float, strikes_range: int = 5):
        """
        ...
        expiry_info = ExpiryInfo(expiry_code=expiry_code, expiry_date=expiry_date, is_weekly=True)

        self.universe = UniverseDefinition(
            underlying=underlying, expiry=expiry_info, atm_reference=atm_reference, strikes_range=strikes_range
        )

        self.logger.info(f"Universe defined: {underlying} {expiry_code} " f"ATM {atm_reference} ±{strikes_range}")
    ...
    def fetch_option_chain(self) -> Optional[OptionChainSnapshot]:
        ...
        try:
            ...
            # Step 2: Filter noise
            filtered_snapshot, drops = self.noise_filter.filter_snapshot(raw_snapshot, self.snapshot_engine.previous)
            ...
            # Step 7: Cache
            self.snapshot_cache.store(self.universe.underlying, self.universe.expiry.expiry_code, filtered_snapshot)
    ...
    def _fetch_from_broker(self) -> Optional[OptionChainSnapshot]:
        """
        Fetch raw option chain from broker via SmartAPI.
        """
        try:
            # Get SmartAPI client
            from src.integrations.angelone.angelone_adapter import get_angelone_adapter

            broker = get_angelone_adapter()

            if not broker._smartapi_client:
                self.logger.warning("Broker not connected; using empty snapshot")
                return None

            # Get spot price
            spot_data = broker._smartapi_client.get_ltp_data(
                exchange="NSE",
                trading_symbol=self.universe.underlying,
                token=self._get_token_for_underlying(self.universe.underlying),
            )

            if not spot_data:
                self.logger.warning("Could not fetch spot price")
                return None

            spot_price = float(spot_data.get("ltp", 0))

            # Search for option symbols
            search_pattern = f"{self.universe.underlying}{self.universe.expiry.expiry_code}"
            search_results = broker._smartapi_client.search_scrip("NFO", search_pattern)

            if not search_results:
                self.logger.warning(f"No options found for {search_pattern}")
                return None

            # Extract unique strikes
            strikes_set = set()
            for scrip in search_results:
                symbol = scrip.get("tradingsymbol", "")
                # Extract strike from symbol (e.g., NIFTY23JAN24000CE -> 24000)
                try:
                    # CE and PE symbols end with CE/PE
                    strike_str = symbol[:-2] if symbol.endswith(("CE", "PE")) else symbol
                    # Extract only digits from end
                    for i in range(len(strike_str) - 1, -1, -1):
                        if not strike_str[i].isdigit():
                            strike_str = strike_str[i + 1 :]
                            break
                    if strike_str:
                        strikes_set.add(int(strike_str))
                except (ValueError, IndexError):
                    continue

            strikes = sorted(list(strikes_set))
            self.logger.info(
                f"Fetched {len(strikes)} strikes from broker: {strikes[:5]}..."
                if len(strikes) > 5
                else f"Fetched strikes: {strikes}"
            )
            ...
            snapshot = OptionChainSnapshot(
                ...
                strikes={},  # Will be populated later
                data_quality_score=1.0 if strikes else 0.0,
            )

            return snapshot

        except Exception as e:
            self.logger.error(f"_fetch_from_broker error: {e}")
            return None

    def _get_token_for_underlying(self, underlying: str) -> str:
        """Get exchange token for underlying symbol."""
        token_map = {"NIFTY": "99926015", "BANKNIFTY": "99926056", "FINNIFTY": "99926065"}
        return token_map.get(underlying, "99926015")
    ...
    def start_continuous_fetch(self, interval_sec: float = 5):
        ...
        self._fetch_thread = threading.Thread(
            target=self._fetch_loop, args=(interval_sec,), daemon=True, name="OptionChainFetch"
        )
    ...
    def _fetch_loop(self, interval_sec: float):
        ...
        while not self._stop_event.is_set():
            try:
                ...
                if snapshot:
                    self.logger.debug(
                        f"Fetched: {snapshot.complete_pairs} complete pairs, " f"{snapshot.fetch_latency_ms:.1f}ms"
                    )
