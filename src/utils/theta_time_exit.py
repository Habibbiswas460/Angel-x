...
from src.utils.exit_models import (
    ThetaDecayMonitor, ThetaExitSignal, Phase7Config
)
...
class ThetaDecayExitEngine:
    ...
    def detect_theta_acceleration(
        ...
        if theta_per_minute < -0.05:  # Losing >0.05 per minute
            ...
            return True, acceleration, \
                   f"Theta accelerating at -{acceleration:.3f}/min"
    ...
    def check_time_exceeded(
        ...
        if elapsed_secs > max_secs:
            ...
            return True, elapsed_secs, \
                   f"Time limit exceeded by {excess}s ({elapsed_secs}s > {max_secs}s)"
    ...
    def detect_iv_crush(
        ...
        if iv_change_percent < -10:
            return True, abs(iv_change_percent), \
                   f"IV crush: {iv_current:.1%} (was {iv_entry:.1%}), -10.0% threshold"
    ...
    def should_exit_theta(
        ...
        if theta_accel and accel_rate > 0.08:
            return True, ThetaExitSignal.THETA_ACCELERATION, 0.95, \
        ...
        if time_exceeded:
            return True, ThetaExitSignal.TIME_WINDOW_EXCEEDED, 0.99, \
        ...
        if iv_crush:
            return True, ThetaExitSignal.IV_CRUSH_DETECTED, 0.90, \
                   f"IV crush {crush_percent:.1f}% - theta bomb safe"
        
        # Check 4: Combination of signals
        if theta_accel and (time_exceeded or iv_crush):
            return True, ThetaExitSignal.THETA_ACCELERATION, 0.98, \
