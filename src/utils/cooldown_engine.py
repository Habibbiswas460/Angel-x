...
class CooldownLogicEngine:
    ...
    def calculate_cooldown_period(
        ...
        if exit_pnl > 0:
            ...
            return cooldown, CooldownReason.PROFITABLE_TRADE, f"Win! Quick re-entry in {cooldown}s"
    ...
    def start_cooldown(
        ...
        stress_cooldown, stress_reason, stress_msg = self.calculate_system_stress_cooldown(consecutive_losses, 0)
    ...
    def get_cooldown_status(self, current_time: datetime) -> Tuple[CooldownState, int, str]:
        ...
        if elapsed < self.cooldown_duration:
            ...
            return CooldownState.ACTIVE, remaining, f"Cooldown active: {remaining}s ({percent:.0f}% complete)"
        else:
