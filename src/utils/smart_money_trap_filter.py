...
class FakeMoveAndTrapFilter:
    ...
    def detect_scalper_trap(
        ...
        logger.debug(
            f"Scalper trap check: OI={oi}, Vol={volume}, "
            f"Trap={is_trap}, Prob={prob:.2f}"
        )
    ...
    def detect_noise_trap(
        ...
        logger.debug(
            f"Noise trap check: Gamma={gamma}, Vol={volume}, "
            f"Trap={is_trap}, Prob={prob:.2f}"
        )
    ...
    def detect_theta_trap(
        ...
        logger.debug(
            f"Theta trap check: Theta={theta}, DTE={days_to_expiry}, "
            f"Trap={is_trap}, Prob={prob:.2f}"
        )
    ...
    def detect_reversal_trap(
        ...
        if previous_price_change is not None:
            ...
            price_reversing = (
                (previous_price_change > 0 and price_change < 0) or
    ...
    def detect_liquidity_trap(
        ...
        logger.debug(
            f"Liquidity trap check: OI={oi}, Position={strike_position}, "
            f"Trap={is_trap}, Prob={prob:.2f}"
        )
    ...
    def comprehensive_trap_check(
        ...
        scalper_trap, scalper_prob = self.detect_scalper_trap(
        ...
        noise_trap, noise_prob = self.detect_noise_trap(
            gamma, volume, previous_volume
        )
        if noise_trap:
            trap_probabilities["noise"] = noise_prob
            reasons.append("Gamma flat + Volume spike (Noise Trap)")
        
        theta_trap, theta_prob = self.detect_theta_trap(
            theta, previous_theta, days_to_expiry
        )
        if theta_trap:
        ...
        liquidity_trap, liquidity_prob = self.detect_liquidity_trap(
            oi, strike_position
        )
        if liquidity_trap:
    ...
    def get_metrics(self) -> Dict:
        ...
        return {
            ...
            "trap_detection_rate": (
                self.traps_detected / max(self.validations_count, 1) * 100
            ),
        }
