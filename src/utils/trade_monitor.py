...
from src.utils.trade_models import ActiveTrade, TradeMonitorUpdate, ExitReason, Phase6Config, RiskLimitStatus
...
class TradeMonitoringEngine:
    ...
    def _check_delta_flip(self, trade: ActiveTrade) -> bool:
        ...
        if trade.delta_at_entry is None or trade.delta_current is None:
            return False
    ...
    def check_force_exit_conditions(
        ...
        if spread_points > self.config.max_spread_points:
            if self.config.force_exit_on_spread_spike:
                return True, ExitReason.FORCED_EXIT, (f"Spread too wide ({spread_points}pts) → Force exit")
