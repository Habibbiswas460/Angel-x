...
from src.utils.market_bias_models import (
    TimeWindow, ThetaVelocityAlert, Phase5Config
)
import logging
    ...
    def analyze_time_window(self, current_time: Optional[time] = None) -> Tuple[TimeWindow, bool, str]:
        ...
        if self.config.morning_start <= current_time < self.config.morning_end:
            return (
                TimeWindow.ALLOWED,
                True,
                "Morning window - Full trading allowed"
            )
        ...
        if self.config.caution_start <= current_time < self.config.caution_end:
            return (
                TimeWindow.CAUTION,
                True,
                "Caution window - Apply high filter, theta rising"
            )
        
        # Check theta danger zone (post 12:00)
        if current_time >= self.config.theta_danger_start:
            return (
                TimeWindow.THETA_DANGER,
                False,
                "Theta danger zone - NO TRADE after 12:00 (theta crush risk)"
            )
        ...
        if current_time < self.config.morning_start:
            return (
                ...
                False,
                "Pre-market - trading not started yet"
            )
        
        return (
            TimeWindow.AFTER_HOURS,
            False,
            "After market hours - trading ended"
        )
...
class VolatilityAndThetaGuard:
    ...
    def analyze_theta_velocity(
        ...
        safe_to_trade = (
            not theta_spike_detected and
            not iv_crush_detected and
            not gamma_exhausted and
            theta_current >= self.config.max_theta_threshold
        )
