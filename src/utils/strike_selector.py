...
class TradeEligibilityEngine:
    ...
    def _check_bias(self, bias_type: BiasType) -> EligibilityCheckResult:
        ...
        reason = "Bias is tradeable" if passed else "Bias is NEUTRAL (no clear direction)"
    ...
    def _check_strength(self, bias_strength: BiasStrength) -> EligibilityCheckResult:
        ...
        score = (
            1.0
            if bias_strength == BiasStrength.EXTREME
            else 0.75 if bias_strength == BiasStrength.HIGH else 0.5 if bias_strength == BiasStrength.MEDIUM else 0.0
        )
        reason = (
    ...
    def _check_time(self, time_window: TimeWindow, time_allowed: bool) -> EligibilityCheckResult:
        """Check 3: Time Window OK"""
        passed = time_allowed
        score = 1.0 if time_window == TimeWindow.ALLOWED else 0.5 if time_window == TimeWindow.CAUTION else 0.0
        reason = f"Time window {time_window.value} (ok)" if passed else f"Time window {time_window.value} (blocked)"
    ...
    def _check_data_health(
        ...
        score = (
            1.0
            if is_green and is_fresh
            else 0.5 if is_green and not is_fresh else 0.2 if data_health == DataHealthStatus.YELLOW else 0.0
        )

        reason = (
            "Data health GREEN and fresh" if passed else f"Data health {data_health.value}, age {data_age_seconds:.1f}s"
        )
...
class DirectionAndStrikeSelector:
    ...
    def select_direction_and_strike(
        ...
        if bias_type == BiasType.BULLISH:
            ...
            return self._select_put(
                ...
                pe_atm_volume,
                pe_atm_minus1_gamma,
                pe_atm_minus1_fresh_oi,
                pe_atm_minus1_volume,
            )
    ...
    def _select_call(
        ...
        # Score ATM
        atm_score = self._calculate_strike_score(ce_atm_gamma, ce_atm_fresh_oi, ce_atm_volume)

        # Score ATM+1
        plus1_score = 0.0
        if ce_atm_plus1_gamma is not None:
            plus1_score = self._calculate_strike_score(ce_atm_plus1_gamma, ce_atm_plus1_fresh_oi, ce_atm_plus1_volume)
    ...
    def _select_put(
        ...
        # Score ATM
        atm_score = self._calculate_strike_score(pe_atm_gamma, pe_atm_fresh_oi, pe_atm_volume)
