...
class SlippageCalculator:
    ...
    def calculate_entry_slippage(
        self, ltp: float, bid: float, ask: float, quantity: int, volatility: str = "normal"
    ) -> Dict:
        """
        ...
        logger.debug(
            f"Entry Slippage: LTP={ltp:.2f}, Spread={spread:.2f} ({spread_percent:.2f}%), "
    ...
    def calculate_exit_slippage(
        ...
        slippage_multiplier = {"low": 0.25, "normal": 0.50, "high": 1.0}.get(volatility, 0.50)
        ...
        logger.debug(
            f"Exit Slippage: LTP={ltp:.2f}, Spread={spread:.2f} ({spread_percent:.2f}%), "
            f"Slippage={slippage_amount:.2f} ({slippage_percent:.2f}%)"
        )
    ...
    def calculate_brokerage_and_taxes(self, entry_price: float, exit_price: float, quantity: int) -> Dict:
        """
