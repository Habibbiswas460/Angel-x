...
class OiGreeksCrossValidator:
    ...
    def validate_chain_alignment(
        ...
        smart_entry_count = sum(
            1 for v in strikes_validation.values() if v.get("signal_type") == "smart_entry"
        )
    ...
    def get_cross_validation_confidence(
        ...
        if (greeks_signal_strength > 0.7 and
            oi_signal_strength > 0.7 and
            volume_signal_strength > 0.7):
            return 0.95
        
        # If two strong, one weak → moderate confidence
        strong_count = sum(1 for s in [greeks_signal_strength, oi_signal_strength,
                                       volume_signal_strength] if s > 0.6)
    ...
    def detect_alignment_divergence(
        ...
        if (directions[0] == directions[1] or
            directions[1] == directions[2] or
        ...
        return {
            ...
            "explanation": f"Full divergence: Greeks={greeks_direction}, "
                          f"OI={oi_direction}, Vol={volume_direction}",
        }
    ...
    def get_metrics(self) -> Dict:
        ...
        return {
            ...
            "alignment_rate": (
                self.aligned_signals / max(total, 1) * 100
            ),
            ...
            "trap_rate": (
                self.traps_detected / max(total, 1) * 100
            ),
        }
