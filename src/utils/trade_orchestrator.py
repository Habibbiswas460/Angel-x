...
from src.utils.position_sizer import (
    ...
    StopLossCalculationEngine,
    TargetCalculationEngine,
    OrderPreparationEngine,
)
from src.utils.order_executor import BrokerInterface, AtomicOrderPlacementEngine
from src.utils.trade_monitor import TradeMonitoringEngine, RiskControlEngine, ForceExitDetector, PositionUpdateTracker
...
class OrderExecutionAndRiskEngine:
    ...
    def place_order(
        ...
        if not response.success or response.linked_orders is None:
            self.circuit_breaker.record_failure(f"Order placement failed: {response.error_message}")
    ...
    def exit_trade(
        ...
        duration_secs = int((datetime.now() - trade.entry_time).total_seconds())
    ...
    def activate_kill_switch(
        ...
        current_ltp = {tid: trade.current_ltp or trade.entry_price for tid, trade in self.active_trades.items()}
