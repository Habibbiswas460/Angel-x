...
from src.utils.option_chain_data_models import (
    StrikeData, OptionChainSnapshot, OptionType
)
...
class NoiseFilter:
    ...
    def detect_frozen_ltp(self, strike: StrikeData, prev_strike: Optional[StrikeData]) -> bool:
        ...
        if (strike.ltp == prev_strike.ltp and 
            strike.volume == prev_strike.volume and
            strike.timestamp and prev_strike.timestamp):
    ...
    def detect_ltp_spike(self, strike: StrikeData, prev_strike: Optional[StrikeData]) -> bool:
        ...
        if jump_percent > self.max_ltp_jump_percent:
            self.logger.warning(
                f"Detected LTP spike: {strike.strike} {strike.option_type} "
                f"jumped {jump_percent:.1f}%"
            )
            return False
    ...
    def detect_oi_spike(self, strike: StrikeData, prev_strike: Optional[StrikeData]) -> bool:
        ...
        if strike.oi_prev and strike.oi_prev > 0:
            ...
            if jump_percent > self.max_oi_jump_percent:
                self.logger.warning(
                    f"Detected OI spike: {strike.strike} {strike.option_type} "
                    f"jumped {jump_percent:.1f}%"
                )
                return False
    ...
    def validate_strike(self, strike: StrikeData, prev_strike: Optional[StrikeData] = None) -> Tuple[bool, Optional[str]]:
    ...
    def filter_snapshot(self, snapshot: OptionChainSnapshot, prev_snapshot: Optional[OptionChainSnapshot] = None) -> Tuple[OptionChainSnapshot, List[str]]:
...
class DataValidator:
    ...
    def check_expiry_match(self, snapshot: OptionChainSnapshot, expected_expiry: str) -> bool:
        ...
        if snapshot.expiry != expected_expiry:
            self.logger.warning(
                f"Expiry mismatch: got {snapshot.expiry}, expected {expected_expiry}"
            )
            return False
        return True
    ...
    def check_completeness(self, snapshot: OptionChainSnapshot, min_pairs: int = 8) -> Tuple[bool, float]:
        ...
        if complete_pairs < min_pairs:
            self.logger.warning(
                f"Insufficient complete pairs: {complete_pairs} < {min_pairs}"
            )
...
class BrokerHiccupDetector:
    ...
    def record_error(self) -> bool:
        ...
        if self.consecutive_errors >= self.error_threshold:
            self.logger.warning(
                f"Broker hiccup detected: {self.consecutive_errors} consecutive errors"
            )
            return True
