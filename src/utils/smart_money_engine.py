...
class SmartMoneyDetector:
    ...
    def set_universe(
        ...
        logger.info(
            f"SmartMoneyDetector universe set: {underlying} @ {atm_strike}, "
            f"DTE={days_to_expiry:.1f}"
        )
    ...
    def get_detailed_status(self) -> DetailedSmartMoneyStatus:
        ...
        return DetailedSmartMoneyStatus(
            ...
            fresh_position_count=len(
                self.fresh_detector.get_fresh_positions_in_chain()
            ),
            signal_confidence=(
                self.current_signal.smart_money_probability
                if self.current_signal else 0.0
            ),
            ...
            last_signal_direction=(
                self.current_signal.recommendation
    ...
    def _generate_signal(
        ...
        high_conviction = sum(
            1 for c in oi_classifications.values()
        ...
        fresh_strength = (
            max(f[1].confidence for f in fresh_positions) if fresh_positions else 0.0
        )
        
        # Cross-validation alignment
        aligned_count = sum(
            1 for cv in cross_validations.values() if cv.get("aligned", False)
        )
        ...
        if oi_conviction > 0.6 and high_conviction > 0:
            long_count = sum(
                1 for c in oi_classifications.values()
                if c["type"] == OiBuildUpType.LONG_BUILD_UP
            )
            short_count = sum(
                1 for c in oi_classifications.values()
                if c["type"] == OiBuildUpType.SHORT_BUILD_UP
            )
        ...
        can_trade = (
            oi_conviction > 0.5 and
            volume_aggression > 0.3 and
            trap_probability < 0.4 and
            alignment_rate > 0.6
        )
        ...
        dominant = [
            strike for (strike, opt_type), vol in volume_states.items()
            if vol["spike_factor"] > 2.0
        ]
        ...
        signal = SmartMoneySignal(
            ...
            data_age_seconds=int(
                (datetime.now() - self.last_update_time).total_seconds()
