...
from src.utils.phase6_order_models import (
    BrokerOrder, LinkedOrders, OrderType, OrderStatus,
    OrderPlacementRequest, OrderPlacementResponse,
    Phase6Config
)
    ...
    def place_atomic_order(
        ...
        if not sl_success or sl_order_id is None:
            ...
            response.error_message = (
                f"CRITICAL: BUY placed ({buy_order_id}) but SL FAILED. "
        ...
        response.error_message = (
            f"✓ Atomic placement successful: "
            f"BUY ({buy_order_id}) + SL ({sl_order_id})"
        )
...
class OrphanedPositionDetector:
    ...
    def detect_orphaned(linked_orders_list: Dict[str, LinkedOrders]) -> list:
        ...
        for buy_id, linked in linked_orders_list.items():
            ...
            if (linked.entry_filled and 
                linked.sl_order.status != OrderStatus.EXECUTED):
