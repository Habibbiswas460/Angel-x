...
class OptionChainView:
    ...
    def get_dominant_strikes(self) -> Dict[str, List[int]]:
        ...
        return {
            'ce_dominant_strikes': ce_dominant,
            'pe_dominant_strikes': pe_dominant
        }
...
class BiasEligibilityPanel:
    ...
    def get_visual_display(self) -> str:
        ...
        allowed_icon = "✅" if self.trade_allowed == TradeAllowance.ALLOWED else ("⚠️" if self.trade_allowed == TradeAllowance.CAUTIOUS else "❌")
...
class LiveTradeMonitor:
    ...
    def get_visual_display(self) -> str:
        ...
        pnl_icon = "🟢" if self.unrealized_pnl > 0 else "🔴"
...
class RiskSafetyPanel:
    ...
    def is_at_risk_limit(self) -> bool:
        ...
        return (
            self.trades_taken_today >= self.max_trades_allowed or
            self.daily_pnl <= -self.max_loss_limit or
            self.drawdown_pct >= 15.0 or
