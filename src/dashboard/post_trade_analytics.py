...
class PostTradeAnalytics:
    ...
    def _generate_insights(self) -> List[str]:
        ...
        if best_exit and best_exit.win_rate > 70:
            insights.append(f"✅ {best_exit.exit_reason.value} exits performing well ({best_exit.win_rate:.1f}%) - use more")
        ...
        if best_time and worst_time:
            insights.append(f"✅ Focus on {best_time.session_name} ({best_time.win_rate:.1f}%), avoid {worst_time.session_name} ({worst_time.win_rate:.1f}%)")
        
        # OI conviction insight
        if self.oi_conviction_report.high_oi_win_rate > 75:
            insights.append(f"✅ HIGH OI conviction very effective ({self.oi_conviction_report.high_oi_win_rate:.1f}%) - prioritize these trades")
