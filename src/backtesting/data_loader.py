...
class TickDataLoader:
    ...
    def load_csv(self, filename: str) -> pd.DataFrame:
        ...
        try:
            df = pd.read_csv(filepath, parse_dates=['time'] if 'time' in pd.read_csv(filepath, nrows=1).columns else ['timestamp'])
    ...
    def convert_ticks_to_ohlcv(
        ...
        try:
            ohlcv = df['ltp'].resample(period).agg({
                'open': 'first',
                'high': 'max',
                'low': 'min',
                'close': 'last',
