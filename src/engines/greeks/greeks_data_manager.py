...
class GreeksDataManager:
    ...
    def _fetch_greeks_for_symbol(
        ...
        try:
            ...
            logger.debug(
                f"Fetched Greeks for {symbol}: Delta={snapshot.delta:.4f}, Gamma={snapshot.gamma:.4f}, IV={snapshot.iv:.2f}"
            )
    ...
    def _fetch_option_chain(
        self, underlying: str, expiry_date: str, exchange: Optional[str] = None, strike_count: int = 5
    ) -> Optional[OptionChainSnapshot]:
    ...
    def clear_stale_cache(self):
        ...
        with self.data_lock:
            # Clear stale Greeks
            stale_symbols = [
                symbol for symbol, snapshot in self.greeks_cache.items() if snapshot.is_stale(max_age_seconds=60)
            ]
            for symbol in stale_symbols:
                del self.greeks_cache[symbol]

            # Clear stale chains
            stale_chains = [key for key, snapshot in self.chain_cache.items() if snapshot.is_stale(max_age_seconds=120)]
