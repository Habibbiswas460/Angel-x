...
class GreeksEngine:
    ...
    def set_universe(self, underlying: str, atm_strike: float, days_to_expiry: float):
        ...
        logger.info(f"Greeks universe set: {underlying} ATM={atm_strike}, " f"expiry in {days_to_expiry:.1f} days")

    def update_from_option_chain(
        self, option_chain_snapshot: Dict, broker_greeks: Optional[Dict] = None, broker_iv: Optional[Dict] = None
    ):
        """
    ...
    def _analyze_greeks(self):
        ...
        # Zone analysis
        self.atm_intelligence = self.zone_detector.analyze_atm_zone(self.atm_strike, self.current_greeks)
    ...
    def _generate_strategy_signal(self):
        ...
        if self.atm_intelligence and self.atm_intelligence.gamma_peak_value > 0:
            signal.acceleration_score = min(self.atm_intelligence.gamma_peak_value / 0.1, 1.0)

        # -------- Theta Pressure --------
        if self.atm_intelligence and self.atm_intelligence.theta_kill_value > 0:
            signal.theta_pressure = min(self.atm_intelligence.theta_kill_value / 2.0, 1.0)
        ...
        if self.previous_greeks and self.previous_oi_data:
            sync_result = self.oi_sync_validator.validate_chain_sync(
                self.current_greeks, self.previous_greeks, self.current_oi_data, self.previous_oi_data
            )
    ...
    def __repr__(self) -> str:
        ...
        return f"GreeksEngine({self.underlying} " f"bias={bias} rec={signal} status={status})"
