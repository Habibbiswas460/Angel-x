...
class GreeksCalculator:
    ...
    @staticmethod
    def _d1(spot: float, strike: float, time_to_expiry: float, volatility: float, rate: float) -> float:
    ...
    def calculate_call_greeks(
        cls, spot: float, strike: float, time_to_expiry: float, volatility: float, rate: float = RISK_FREE_RATE
    ) -> Dict[str, float]:
        """
    ...
    def calculate_put_greeks(
        ...
        if time_to_expiry <= 0:
            # Expiry today
            return {"delta": 0.0 if spot > strike else -1.0, "gamma": 0.0, "theta": 0.0, "vega": 0.0}
        ...
        for _ in range(10):  # Max 10 iterations
            try:
                if option_type == OptionType.CALL:
                    greeks = GreeksCalculator.calculate_call_greeks(spot, strike, time_to_expiry, iv, rate)
                else:
                    greeks = GreeksCalculator.calculate_put_greeks(spot, strike, time_to_expiry, iv, rate)
...
class BrokerGreeksValidator:
    ...
    def validate_greek_value(
        greek_name: str, value: Optional[float], option_type: OptionType
...
class GreeksCalculationEngine:
    ...
    def get_metrics(self) -> Dict:
        ...
        return {
            ...
            "success_rate": ((self.calculation_count - self.error_count) / max(self.calculation_count, 1) * 100),
        }
