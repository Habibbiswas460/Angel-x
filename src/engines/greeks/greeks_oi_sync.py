...
from .greeks_models import GreeksSnapshot, OptionType, GreeksOiSyncResult
...
class GreeksOiSyncValidator:
    ...
    def __init__(
        ...
        logger.info(f"Greeks-OI Sync Validator initialized " f"(oi_threshold={oi_change_threshold*100:.1f}%)")
    ...
    def validate_strike_sync(
        ...
        delta_direction = self._get_direction_change(current_greek.delta, previous_greek.delta)

        oi_direction = self._get_direction_change(current_oi_data.oi, previous_oi_data.oi)
        ...
        # -------- Rule 2: Delta ↑ + OI ↓ = Fake Move (DANGER) --------
            ...
            logger.warning(f"FAKE MOVE DETECTED at strike {current_greek.strike}: " f"Delta ↑ but OI ↓")
        ...
        if theta_spiked:
            ...
            if result.recommendation != "AVOID":
                result.recommendation = "CAUTION"
            logger.warning(f"Theta spike detected at strike {current_greek.strike}: " f"Θ={current_greek.theta:.4f}")
            self.theta_exits += 1
    ...
    def validate_chain_sync(
        ...
        for strike, current_greek in current_greeks.items():
            ...
            result = self.validate_strike_sync(current_greek, previous_greek, current_oi, previous_oi)
    ...
    def get_metrics(self) -> Dict:
        ...
        return {
            ...
            "fake_move_ratio": (self.fake_moves_detected / max(self.total_validations, 1)),
            "smart_money_ratio": (self.smart_money_signals / max(self.total_validations, 1)),
        }
