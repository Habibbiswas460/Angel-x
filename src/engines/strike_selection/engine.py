...
class OptionStrike:
    ...
    def is_liquid(self) -> bool:
        ...
        return (
            self.volume >= config.MIN_VOLUME_THRESHOLD
            and self.oi >= config.MIN_OI_THRESHOLD
            and self.bid > 0
            and self.ask > 0
        )
...
class StrikeSelectionEngine:
    ...
    def scan_and_select_best_strike(
        self, available_strikes: List[OptionStrike], bias: str, current_underlying_price: float
    ...
    def scan_strikes_for_bias(
        self, call_strikes: List[OptionStrike], put_strikes: List[OptionStrike], bias: str
    ) -> Optional[OptionStrike]:
        """
