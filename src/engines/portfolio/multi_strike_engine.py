...
class MultiStrikePortfolioEngine:
    ...
    def scan_strike_ladder(
        ...
        for offset in range(-self.strike_range, self.strike_range + 1):
            ...
            try:
                greeks = greeks_manager.get_greeks(
                    symbol=symbol, exchange="NFO", underlying_symbol=underlying, underlying_exchange="NSE"
                )
                ...
                logger.debug(
                    f"Strike {strike}: Δ={greeks.delta:.3f}, Γ={greeks.gamma:.4f}, "
        ...
        if candidates:
            best = candidates[0]
            logger.info(
                f"  Best strike: {best.strike} (Score={best.greeks_score:.1f}, "
    ...
    def build_multi_strike_plan(
        ...
        if not self.enabled:
            ...
            return [LegPlan(strike=atm_strike, option_type=option_type, action="BUY", weight=1.0)]
        ...
        if not candidates:
            ...
            return [LegPlan(strike=atm_strike, option_type=primary_type, action="BUY", weight=1.0)]
        ...
        for i, candidate in enumerate(candidates[:max_legs]):
            ...
            logger.info(
                f"Leg {i+1}: {action} {candidate.option_type} {candidate.strike} "
            ...
            if self.options_helper:
                try:
                    offset_label = self.options_helper.compute_offset(
                        underlying=underlying, expiry_date=expiry_date, strike=leg.strike, option_type=leg.option_type
                    )
                except Exception:
                    offset_label = "ATM"
            result.append(
