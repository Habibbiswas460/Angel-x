...
class MarketData:
    ...
    def __init__(self):
        self.client = api(api_key=config.OPENALGO_API_KEY, host=config.OPENALGO_HOST, ws_url=config.OPENALGO_WS_URL)
    ...
    def get_history(self, symbol, interval, start_date, end_date, exchange=None):
        ...
        try:
            ...
            response = self.client.history(
                symbol=symbol, exchange=exchange, interval=interval, start_date=start_date, end_date=end_date
            )
