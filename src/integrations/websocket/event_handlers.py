...
class PriceUpdateHandler(EventHandler):
    ...
    def __init__(
        ...
        price_threshold: float = 0.5  # % change threshold
    ):
        ...
        try:
            ...
            if token in self.last_prices:
                ...
                if abs(change_pct) >= self.price_threshold:
                    logger.info(
                        f"Price alert: {token} changed {change_pct:.2f}% "
                        f"({last_price:.2f} → {ltp:.2f})"
                    )
...
class GreeksUpdateHandler(EventHandler):
    ...
    def __init__(
        self,
        on_greeks_alert: Optional[Callable] = None,
            ...
            if token in self.last_greeks:
                ...
                if delta is not None and last.get('delta') is not None:
                    ...
                    if delta_change >= self.delta_threshold:
                        logger.warning(
                            f"Delta alert: {token} delta changed by {delta_change:.3f}"
                        )
                ...
                if gamma is not None and last.get('gamma') is not None:
                    ...
                    if gamma_change >= self.gamma_threshold:
                        logger.warning(
                            f"Gamma alert: {token} gamma changed by {gamma_change:.4f}"
                        )
...
class OrderBookHandler(EventHandler):
    ...
    def __init__(
        self,
        on_spread_alert: Optional[Callable] = None,
        spread_threshold: float = 1.0  # % spread threshold
    ):
        ...
        try:
            ...
            if best_bid > 0 and best_ask > 0:
                ...
                if spread >= self.spread_threshold:
                    logger.warning(
                        f"Wide spread alert: {token} spread = {spread:.2f}% "
...
class VolumeHandler(EventHandler):
    ...
    def __init__(
        self,
        on_volume_spike: Optional[Callable] = None,
        spike_multiplier: float = 2.0  # Volume spike threshold
    ):
        ...
        try:
            ...
            if len(self.volume_history[token]) >= 20:
                ...
                if volume > avg_volume * self.spike_multiplier:
                    logger.info(
                        f"Volume spike: {token} volume {volume} "
                        f"({volume/avg_volume:.1f}x average)"
                    )
