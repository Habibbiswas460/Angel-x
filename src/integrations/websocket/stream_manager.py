...
class StreamManager:
    ...
    def __init__(self, websocket_client: WebSocketClient, buffer_size: int = 1000):
        """
        ...
        # Data buffers
        self.tick_buffers: Dict[str, deque] = defaultdict(lambda: deque(maxlen=buffer_size))
        ...
        # Instrument master cache for token lookup
        self._instruments: Optional[Dict] = None

        logger.info("StreamManager initialized")

    def subscribe_instruments(self, instruments: List[Dict[str, Any]], mode: SubscriptionMode = SubscriptionMode.QUOTE):
        """
        ...
        try:
            ...
            for exchange, tokens in exchange_groups.items():
                self.ws_client.subscribe(tokens=tokens, mode=mode, exchange=exchange)
    ...
    def subscribe_option_chain(
        self, underlying: str, expiry: str, strikes: List[int], mode: SubscriptionMode = SubscriptionMode.QUOTE
    ):
        """
    ...
    def _get_option_token(self, underlying: str, expiry: str, strike: int, option_type: str) -> str:
        """
        Get option token from instrument master.

        Args:
            underlying: NIFTY, BANKNIFTY, etc.
            expiry: Expiry date code (e.g., '23JAN26')
            strike: Strike price
        ...
        """
        try:
            # Build symbol name (e.g., NIFTY23JAN2624000CE)
            symbol = f"{underlying}{expiry}{strike}{option_type}"

            # Lookup from cached instruments if available
            if self._instruments and symbol in self._instruments:
                token = self._instruments[symbol]["token"]
                logger.debug(f"Token found: {symbol} -> {token}")
                return token

            # Fallback: return placeholder if not found
            logger.warning(f"Token not found for {symbol}; using placeholder")
            return f"TOKEN_{symbol}"

        except Exception as e:
            logger.error(f"_get_option_token error: {e}")
            return f"{underlying}_{expiry}_{strike}_{option_type}"

    def load_instrument_master(self, instruments: Dict[str, Dict]):
        """Load instrument master data for token lookups.

        Args:
            instruments: Dict mapping symbol to instrument data
        """
        self._instruments = instruments
        logger.info(f"Loaded {len(instruments)} instruments for token lookup")
