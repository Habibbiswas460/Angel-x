...
class RiskManager:
    ...
    def update_portfolio_greeks(
        ...
        net_theta: float,
        net_vega: float,
        gross_delta: Optional[float] = None
    ):
        """
        ...
        with self.risk_lock:
            ...
            if position_delta is not None:
                ...
                if abs(new_net_delta) > self.greeks_limits.max_net_delta:
                    return False, f"Would exceed max net delta: {new_net_delta:.1f} > {self.greeks_limits.max_net_delta}"
                
                new_gross_delta = self.current_gross_delta + abs(position_delta)
                if new_gross_delta > self.greeks_limits.max_gross_delta:
                    return False, f"Would exceed max gross delta: {new_gross_delta:.1f} > {self.greeks_limits.max_gross_delta}"
            ...
            if position_gamma is not None:
                ...
                if abs(new_net_gamma) > self.greeks_limits.max_net_gamma:
                    return False, f"Would exceed max net gamma: {new_net_gamma:.2f} > {self.greeks_limits.max_net_gamma}"
            ...
            if position_theta is not None:
                ...
                if new_net_theta < self.greeks_limits.max_daily_theta:
                    return False, f"Would exceed max theta decay: {new_net_theta:.2f} < {self.greeks_limits.max_daily_theta}"
